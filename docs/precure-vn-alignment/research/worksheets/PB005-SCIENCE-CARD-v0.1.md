...
| Đại lượng | Vai trò proxy | Giữ hôm nay | Bỏ |
|-----------|---------------|-------------|-----|
| **CFU / biomass** | Tải / sinh khối | Chuỗi \(t_0,t_{\text{mid}},t_{\text{end}}\) | Snapshot cuối = “early” |
| **Viability** | Sống sót sau tiếp xúc | Time-course | = diệt biofilm lâm sàng |
| **EPS / matrix** | Phenotype biofilm | Gated / sau G2 nếu omics | Map thẳng Dx BN |
| **Contact time** | Chế phẩm × thời gian | Ghi rõ trong protocol lab | Ngoại suy primary D21 |
| **SA-03** | In-vitro proxy | Không = RCT Smart A | Cite support SA-01 \(Y\) |

## Phương trình PB-005

```text
Z_lab(t') chuỗi ≥2  ·  hỏi map “chuyển pha”  ·  giới hạn ngoại suy
SA-03 = proxy in-vitro  ≠  RCT  ≠  Dx BN  ≠  primary D21
Snapshot cuối  ≠  early-signal biofilm
PB-005 drill  ≠  order X người / mở G2
```
...
Order PEA/swab người vì PB-005? KHÔNG
1 việc ≤30′ (EH-SA03 / TRANSLATION / SYNTH / PB006): ________
Đóng Goal vì PB-005? KHÔNG
```
...
`problem-bank` PB-005 · `EH-SA03-ZX-variables` · `PB005-5MIN` · `PB005-EQ-5MIN` · `TRANSLATION-SCIENCE-CARD` · `SYNTH-SCIENCE-CARD` · `PB006-SCIENCE-CARD` · `OMICS-GATES-SCIENCE-CARD` · `CROSS-SA-SCIENCE-CARD` · `SCIENCE-CARDS-INDEX` · `DAILY-STACK-AFTER-STREAK3` · **`PB001-SCIENCE-CARD`**
