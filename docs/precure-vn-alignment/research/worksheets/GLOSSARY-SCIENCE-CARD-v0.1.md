...
| Term | Nghĩa Smart A (giữ) | Không nghĩa (bỏ) |
|------|---------------------|------------------|
| \(t^*\) | Primary D21/D3/D14 | “Trước triệu chứng” marketing |
| \(t'\) | Exploratory trước \(t^*\) | Thay primary |
| Early-signal | \(Z(t')\) → \(Y(t^*)\) SAP ES | App Dx / auto-treat |
| \(Z\) | Lâm sàng dọc | Omics mặc định |
| \(X\)/L3 | Omics gated | Order vì thuộc glossary |
| L1–L3 | Tầng PB-009 | L3 = đã đủ vì L1 |
| G2 | Omics gate CLOSED | Pass bằng sandbox |
| PREP/DONE | Agent scaffold / PI tick | PREP = STREAK DONE |

## Phương trình GLOSSARY

```text
1 term  +  nghĩa Smart A  +  1 “không nghĩa”  ≥  trước  claim press/VDHN
...
`EARLY-SIGNAL-GLOSSARY` · `GLOSSARY-5MIN` · `GLOSSARY-EQ-5MIN` · `SHIFT-SCIENCE-CARD` · `MEDIA-SCIENCE-CARD` · `EPI-SCIENCE-CARD` · `L1L2L3-SCIENCE-CARD` · `EQ-SCIENCE-CARD` · `SCIENCE-CARDS-INDEX` · **`MEDIA-BOUND-SCIENCE-CARD`** · `DAILY-STACK-AFTER-STREAK3` · **`CROSS-SA-SCIENCE-CARD`**
