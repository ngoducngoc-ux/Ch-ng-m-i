...
|------------|--------|---------|
| **PREP** | File/log/worksheet có khung · insight chưa do PI | Agent (scaffold) |
| **DONE** | Insight lời PI + 1 câu hỏi SA/y tế số + việc nhỏ · DOI thật | **PI** |
| **MISS** | Không log trong ngày lịch | PI ghi makeup |

## Không tính DONE (dán log nếu nghi)

```text
verify.sh PASS / CI xanh ≠ DONE
Thêm daily PREP / science-card densify ≠ DONE
Synthetic AUROC / sandbox Python ≠ DONE
Agent tick DONE thay PI ≠ DONE
UpdateGoal complete trên PREP = CẤM
```

## Checklist 15′ (trước tick)
...
STREAK_TRACKER hàng ngày → DONE (PI)? ________
Agent scaffold hôm nay = STREAK DONE? KHÔNG
verify/CI/AUROC sandbox = DONE? KHÔNG
Đóng Goal vì densify/PREP? KHÔNG
1 việc ≤30′ (FILL-AID / NATMED / script STREAK3): ________
```
