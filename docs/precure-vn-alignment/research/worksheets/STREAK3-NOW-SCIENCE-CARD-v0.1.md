# STREAK3-NOW — thẻ khoa học 1 trang (mở ngay · 4 bước · tick 19/09)

**Mã:** STREAK3-NOW-SCIENCE-CARD-v0.1  
**Ngày:** 2026-09-16  
**Neo:** `STREAK3-NOW-1PAGE` · STREAK3-PACK · FILL-AID · PI-SESSION · NATMED · RITUAL-DONE  
**Dùng khi:** STREAK thật **&lt;3** · cần **1 trang mở ngay** không scroll hub · trước tick **19/09**  
**Ưu tiên STREAK&lt;3:** **`STREAK3-PACK-SCIENCE-CARD`** · NATMED · ALERT · HAWTHORNE · MEDIA family · RITUAL-DONE · PI-SESSION · FILL-AID → tick **19/09**  
**Goal:** ACTIVE · **chỉ PI** tick DONE · agent ≠ tick · L3 CLOSED · PREP ≠ DONE  

## Mục đích

Ôn **NOW = cửa sổ chống quên**: 7 file mở song song + đúng 4 bước 45′. Khác `STREAK3-PACK` (thứ tự pack) · `PI-SESSION` (phút×thẻ chi tiết) — thẻ này giữ **mở ngay + xong khi** tối giản.

**Mở song song:** thẻ này · `STREAK3-NOW-1PAGE` · `STREAK3-FILL-AID` · `NATMED-STREAK3-SCIENCE-CARD` · log `2026-09-19.md` · `STREAK_TRACKER`

## 4 bước → giữ / bỏ

| Bước | Việc | Giữ (xong khi) | Bỏ |
|------|------|----------------|-----|
| **1** 0–15 | Abstract Nat Med + thẻ | 3 ý: lặp · actionable · lifestyle | Scroll 120 PREP |
| **2** 15–30 | 1 hàng A1–A4 → log | insight lời PI · ALERT A__ | App Dx |
| **3** 30–35 | Tick tracker 19/09 | **PI** PREP→DONE · STREAK≥3 | Agent/CI tick |
| **4** 35–45 | VDHN≠DOI | 1 câu trong log | Press = RCT evidence |

## Điền nhanh (dán log)

```text
ALERT map hôm nay: A1|A2|A3|A4 — ________
1 câu lệch hướng ≤25 từ (tự viết): ________
1 câu VDHN ≠ DOI: ________
STREAK_TRACKER 2026-09-19: PREP → DONE? (chỉ PI) ________
Mở DAILY-STACK hôm nay? CHƯA (chỉ sau STREAK≥3)
Đóng Goal / order omics? KHÔNG
```

## Checklist 15′

```text
STREAK DONE hiện: ________ (cần ≥3)
Đã mở NOW-1PAGE + FILL-AID + NATMED thẻ? ________
Gate RITUAL-DONE skim? ________
Agent tick DONE? KHÔNG
verify/densify = DONE? KHÔNG
Mở OPENER/DAILY-STACK nếu STREAK<3? CHƯA
1 việc ≤30′ sau tick: AFTER-OPENER | makeup Zhou — ________
```

## Đừng nhầm atlas

| Thẻ / atlas | Việc |
|-------------|------|
| **thẻ này** | Khoa học 1 trang · mở ngay + 4 bước |
| `STREAK3-NOW-1PAGE` | Bản anti-forget gốc |
| `STREAK3-PACK-SCIENCE-CARD` | Thứ tự pack path |
| `PI-SESSION-SCRIPT-STREAK3-SCIENCE-CARD` | Phút×thẻ chi tiết |
| `STREAK3-FILL-AID` | Paste A1–A4 |
| `AFTER-STREAK3-OPENER` | **Chỉ sau** STREAK≥3 |

## Cấm

- Agent / CI / Drive / densify = STREAK DONE  
- Coi mở NOW = DONE · UpdateGoal complete trên PREP  
- Order omics / biospecimen / đóng Goal  

## Liên kết

`STREAK3-NOW-1PAGE` · `STREAK3-PACK-SCIENCE-CARD` · `STREAK3-FILL-AID` · `PI-SESSION-SCRIPT-STREAK3-SCIENCE-CARD` · `NATMED-STREAK3-SCIENCE-CARD` · `RITUAL-DONE-vs-PREP-SCIENCE-CARD` · `PI-NEXT-45MIN` · `AFTER-STREAK3-OPENER-1PAGE` · `SCIENCE-CARDS-INDEX` · `DAILY-STACK-AFTER-STREAK3` · `STREAK_TRACKER`
