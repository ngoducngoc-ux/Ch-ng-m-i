# STREAK3 FILL-AID — dán sẵn (PI · chống quên)

**Mã:** STREAK3-FILL-AID-v0.1  
**Ngày:** 2026-09-16  
**STREAK thật:** **2 DONE** · cần **≥3** · hàng `2026-09-19` còn **PREP**  
**Dùng với:** `STREAK3-NOW-1PAGE` · log `2026-09-19.md` · `STREAK_TRACKER`  
**Goal:** ACTIVE · **chỉ PI** tick DONE · agent ≠ tick · PREP ≠ DONE  

## Cách dùng (≤5′ sau abstract)

1. Mở DOI abstract + `NATMED-STREAK3-SCIENCE-CARD` + `STREAK3-NOW-1PAGE`  
2. Chọn **1 khối A1–A4** bên dưới → dán vào log insight  
3. Dán **1 câu lệch** + **1 câu VDHN≠DOI** (đã viết sẵn — chỉnh nếu muốn)  
4. Tick DONE log + đổi tracker hàng **2026-09-19** PREP → **DONE**  

## Chọn 1 ALERT (dán nguyên khối)

### A1 — PCT đứng + CFU không giảm

```text
ALERT map hôm nay: A1 — PCT_EPITH đứng + CFU không giảm → hội chẩn / cấy lại SOP (nội bộ nghiên cứu).
1 câu lệch hướng ≤25 từ: Tín hiệu sớm trên Z dọc trước D21 ≠ sản phẩm Dx thương mại.
```

### A2 — VAS↑ + AE_LOCAL

```text
ALERT map hôm nay: A2 — VAS↑ + AE_LOCAL → đánh giá AE / Ch.7 (hành động nội bộ, không app tự đổi nhánh).
1 câu lệch hướng ≤25 từ: ALERT có hành động chăm sóc cohort ≠ dashboard vanity.
```

### A3 — PCT lùi D0→D7

```text
ALERT map hôm nay: A3 — PCT lùi D0→D7 → theo dõi quỹ đạo + clin_event (không train AI cá thể từ 1 điểm).
1 câu lệch hướng ≤25 từ: Quỹ đạo PCT sớm ≠ claim “đã phát hiện sớm như press”.
```

### A4 — CLIN_EVENT + PCT đứng

```text
ALERT map hôm nay: A4 — CLIN_EVENT + PCT đứng → ghi sự kiện + SOP đo lại (không ngoại suy Dx).
1 câu lệch hướng ≤25 từ: Sự kiện chuyển trạng thái trong RCT ≠ chứng minh sản phẩm Precure.
```

## VDHN ≠ DOI (dán 1 lần)

```text
1 câu VDHN ≠ DOI: VDHN/Precure.LLC = tầm nhìn multi-omics+AI; SA-01 hiện chỉ ALERT nội bộ A1–A4 trên Z REDCap — Nat Med minh họa “actionable trong cohort”, không chứng minh Dx sẵn sàng.
```

## Tracker (chỉ PI)

| File | Việc |
|------|------|
| `daily-log/2026-09-19.md` | Check Đã đọc + STREAK DONE |
| `STREAK_TRACKER.md` | Hàng `2026-09-19` · cột Trạng thái: **PREP → DONE** |

Sau tick: STREAK thật ≥3 → mở **`AFTER-STREAK3-OPENER-1PAGE`** (phiên đầu) · rồi `DAILY-STACK-AFTER-STREAK3` · **không** đóng Goal · **không** order omics.

## Cấm

- Agent / EQ densify / Drive / Calendar = STREAK DONE  
- Coi 120 PREP = xong · UpdateGoal complete  

## Liên kết

`STREAK3-NOW-1PAGE` · **`NATMED-STREAK3-SCIENCE-CARD`** · `PI-SESSION-SCRIPT-STREAK3` · `NATMED-ACTIONABLE-ALERT-MAP` · `MEDIA-5MIN` · `PI-NEXT-45MIN`
