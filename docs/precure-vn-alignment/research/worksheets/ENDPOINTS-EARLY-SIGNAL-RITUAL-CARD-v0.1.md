# Endpoints × early-signal — thẻ ritual (Ngày 08–14)

**Mã:** ENDPOINTS-EARLY-SIGNAL-RITUAL-CARD-v0.1  
**Ngày:** 2026-09-16 · **Cập nhật:** bridge Endpoints cross-SA · EQ-02/05 M0–M3  
**Sau:** Nat Med · PEA L1→L2→L3 card · **Cờ đầu:** SA-01 · G2 **CLOSED**  
**Study sheet:** `study-sheets/STUDY-SHEET-SMART-A-ENDPOINTS-v0.1.md`  
**Bridge 1 trang:** `ENDPOINTS-CROSS-SA-BRIDGE-v0.1.md`

## Một câu (mọi ngày tuần B)

> Early-signal Smart A = chuỗi \(Z\) trước \(t^*\) (exploratory SAP ES) — **không** đổi primary, **không** Dx app, **không** mở \(X\) trước G2; SA-03/04 là cổng in-vitro/ISO, không thay RCT người.

## Ma trận nhanh \(t^*\) → \(Z\) sớm

| SA | \(t^*\) | \(Z\) sớm (ôn) | Cấm suy diễn |
|----|---------|----------------|--------------|
| **01** | D21 biểu mô | D0–D7 PCT/CFU/VAS/`clin_event` | AUROC sandbox = lâm sàng |
| **02** | VAS D3 | Series D0–D3 · marker gated | Marker = primary |
| **03** | Biofilm ATCC | — | In-vitro = Dx BN |
| **04** | ISO 10993 | — | Omics người trước contact/ISO |
| **05** | PUSH D14 | D0–D7 PUSH + TURN_ADHERE | \(\Delta\)PUSH = app ICU Dx |

## Ritual từng ngày (1 câu DONE)

| N | Log | Quyết định ghi vào log | Artifact |
|---|-----|------------------------|----------|
| **08** | `2026-09-24.md` | Component PUSH vs total — early có thể trên component | `PUSH-SA05-COMPONENTS` · Stotts DOI |
| **09** | `2026-09-25.md` | \(\Delta\)PUSH từ chuỗi D0–D7 ≠ app Dx | `EQ-SA05` (M0–M3) · `EQ-EH-SA05-GAP` · bridge |
| **10** | `2026-09-26.md` | \(Z\) D0–D7 exploratory vs primary D21 | `EQ-SA01` · **`EPI-SA01-EARLY-WINDOW`** · `GAP-SA01-eCRF` |
| **11** | `2026-09-27.md` | Weekly: 3 insight · giữ cờ SA-01 | `weekly-2026-09-27.md` · bridge ma trận |
| **12** | `2026-09-28.md` | VAS D3 primary; 0–10 vs mm · M1\*≠VAS_D3 | `EQ-SA02` · **`VAS-SCALE-HARMONIZE`** drill 8′ |
| **13** | `2026-09-29.md` | Biofilm in-vitro ≠ early-signal BN | `SA03-BIOFILM-TRANSLATION` · gates |
| **14** | `2026-09-30.md` | ISO / SA-04 trước mọi L3 người | `ISO-SWAB-CONTACT-PRIORITY` · `EH-SA04-gates` |

## Y tế số / AI (PB-009 — nhắc mỗi ngày)

```text
L1 visit + clin_event  →  L2 M0–M3 (EQ theo SA)  →  L3 X chỉ sau G2
```

`AI-LONGITUDINAL-STACK` · `y-te-so-precure-bridge` · `MULTI-OMICS-GATES`

## Checklist DONE (PI)

- [ ] Study sheet endpoints + **1 hàng** bảng ritual trên  
- [ ] 1 câu quyết định trong log ngày N  
- [ ] STREAK PREP → **DONE**  
- [ ] Cờ đầu vẫn **SA-01** (SA-02/05 support; 03/04 cổng)

## Sau Ngày 14

Tier 2 SPIRIT/TT43/TRIPOD: `STUDY-SHEET-DESIGN-YTESO-AI-v0.1.md` · `TIER-2-30DAY-HANDOFF.md` · checkpoint **16/10**

## Liên kết

- **Bridge:** `ENDPOINTS-CROSS-SA-BRIDGE-v0.1.md` · EQ `EQ-SA01|02|05`  
- PEA trước đó: `PEA-L1L2L3-DECISION-CARD-v0.1.md` · `MULTI-OMICS-PEA-SA01-BRIDGE`  
- Nat Med actionable: `NATMED-ACTIONABLE-ALERT-MAP-v0.1.md`  
- Handoff: `TIER-1-7DAY-HANDOFF.md` Tuần B
