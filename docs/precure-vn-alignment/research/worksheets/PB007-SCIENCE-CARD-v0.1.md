...
| Bước | Vai trò | Giữ hôm nay | Bỏ |
|------|---------|-------------|-----|
| **\(Y(t^*)\)** | Endpoint đã chốt | SA-01 D21 / 02 VAS D3 / 05 PUSH D14 | Đổi primary vì ES |
| **\(t'\)** | Cửa sổ sớm | \(t' \ll t^*\) | \(t'=t^*\) làm “early” |
| **M0–M3 / \(Z\)** | Lâm sàng dọc | Trả lời **trước** \(X\) | Nhảy thẳng M4 |
| **M4 / \(X\)** | Omics gated | **CLOSED** đến G2+ethics+\(X\) thêm giá trị | Mở vì khung “đã đủ” |
| **Sandbox AUROC** | Pipeline QC | ≠ bằng chứng \(X\) thêm giá trị | Đóng PB-007 |

## Phương trình PB-007

```text
P(Y=1) = σ(β0 + β_Z·Z(t') + …)
         rồi (chỉ sau G2):  + β_X·X(t')
Z(t') trước  ·  X sau G2  ·  không đảo thứ tự
Khung phương trình “đủ”  ≠  order PEA  ≠  AUROC sandbox = evidence
```
...
| Thẻ / atlas | Việc |
|-------------|------|
| **thẻ này** / PB-007 framework | Thứ tự \(Z\)→\(X\) · khung phương trình |
| `EQ-SCIENCE-CARD` | Ladder M0–M3 thao tác |
...
`problem-bank` PB-007 · `PB-007-equation-framework` · `PB007-5MIN` · `PB007-EQ-5MIN` · `SHIFT-PB007-5MIN` · `EQ-SCIENCE-CARD` · `SHIFT-SCIENCE-CARD` · `G2-SCIENCE-CARD` · `L1L2L3-SCIENCE-CARD` · `PB001-SCIENCE-CARD` · `PB002-SCIENCE-CARD` · `PB003-SCIENCE-CARD` · `SCIENCE-CARDS-INDEX` · `DAILY-STACK-AFTER-STREAK3`
